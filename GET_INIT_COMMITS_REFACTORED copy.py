'''
1st part getProjectNames(sstubsLarge_fn, project_names_dict_fn) :
    - get project names from sstubsLarge
    - save into file > /home/elia/SSTUBS_PAPER/results/project_names_dict.json

VERSION FROM 04/12/21 AT 11:30 PM
'''
import os
...
formerCommitMatchesFN = '/home/elia/SSTUBS_PAPER/results/JmeterFormerCommitsMap_NEW.json'
...
def write_to_json(writeThis, filename = '/home/elia/SSTUBS_PAPER/results/FINAL_SSTUBS.json', mode = 'a'):
...
def set_nulls (sstub):
    ...
    # sstub['initCommit'] = 'NaN'
    # sstub['initAuthor'] = 'NaN'
    # sstub['initDate'] = 'NaN'
    # sstub['whereAbouts'] = 'NaN'
    return sstub
...
def get_init_commit(git_source_potential_commits, sstub, curr_buggycode, gitsRedownloaded, gitsTriedFailedRedownload):
    # output1 = subprocess.check_output(git_show, shell=True)
    ...
    for commit in git_source_potential_commits:
        ...
        output = str(output)
            # os.system(git_show)
            # if output.find('fatal: bad object') > -1:
            #     delete_and_clone(project_formatted_name, project, gitsRedownloaded, gitsTriedFailedRedownload)
    ...
    # curr_buggycode = [ line for line in lines if  line.replace(" ", "").find(curr_sourceBeforeFix.replace(" ", "")) != -1 ]
        ...
        if len (lines_filtered) != 0:
                ...
                # Author: pranay <lourdupranay@gmail.com>
                # Date:   Sat May 20 18:27:05 2017 +0530
            ###################################################################
            # GET INFO FOR commit WITH OS.SYSTEM
            # AUTHOR EMAIL %ae
            # AUTHOR DATE %ai
            ...
            # COMMIT NOTES %N
            ###################################################################
            # author = git_show_conts[1].split('<')
            # author = author[1].strip('>')
            # date = git_show_conts[2].strip('Date:').strip()
...
def get_final_files(sstubs):
    FP = r'/home/elia/SSTUBS_PAPER/results/FINAL_SSTUBS_DEBUGGED_FORCSV.json'
    FP1 = r'/home/elia/SSTUBS_PAPER/results/FINAL_SSTUBS_NOPOSSIBLECOMMITS.json' # NO POSSIBLE COMMITS
    FP2 = r'/home/elia/SSTUBS_PAPER/results/FINAL_SSTUBS_NOFINALCOMMIT.json' # NO FINAL COMMIT
    FP3 = r'/home/elia/SSTUBS_PAPER/results/FINAL_SSTUBS_ALLINFO.json' # PERFECT

    noPossibleCommits = []
    noFinalCommit = []
    perfect = []
    # results of sstubs logic
    filePath1 = r'/home/elia/SSTUBS_PAPER/results/FINAL_SSTUBS.json'
    ...
    # filePath2 = r'/home/elia/SSTUBS_PAPER/projects/FINAL_SSTUBS_DEBUGGED_FORCSV.json'
    ...
    generateFinalCSVs(FP1, '/home/elia/SSTUBS_PAPER/results/FINAL_SSTUBS_NOPOSSIBLECOMMITS.csv')
    generateFinalCSVs(FP2, '/home/elia/SSTUBS_PAPER/results/FINAL_SSTUBS_NOFINALCOMMIT.csv')
    generateFinalCSVs(FP3, '/home/elia/SSTUBS_PAPER/results/FINAL_SSTUBS_ALLINFO.csv')
...
def main_logic(projectNames, sstubs, gitsRedownloaded, gitsTriedFailedRedownload):
    ...
    for project, project_formatted_name in projectNames.items():
        ...
        # if project.strip() == 'android-bootstrap' or project_formatted_name.strip() == 'android-bootstrap':
        #     continue
        ...
        for sstub in sstubs:
            ...
            # if process breaks implement this:
            # if sstub['projectName'] in done.keys():
            #     if sstub['fixCommitSHA1'] in done[sstub['projectName']]:
            #         continue

            # fixDate = get_fix_date(sstub)
            # # sstub['fixDate'] = fixDate.replace("'b",'').replace("\\n'",'')
            # sstub['fixDate'] = fixDate
            # get fix commit info
...
# with open('/home/elia/SSTUBS_PAPER/results/project_names_dict.json', 'r') as f:
